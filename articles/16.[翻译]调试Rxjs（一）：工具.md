...
![banner](https://github.com/AngularInDepth/angularindepth/image/16/banner.jpeg?raw=true)
...
![Console_1](https://github.com/AngularInDepth/angularindepth/image/16/1.png?raw=true)

要显示特定标记的 `Observable` 的信息，可以将标记名称或者正则表达式传递给 `show`：

![Console_2](https://github.com/AngularInDepth/angularindepth/image/16/2.png?raw=true)

可以通过调用 `rxSpy.log` 来显示被标记的 `Observable` 的日志信息：

![Console_3](https://github.com/AngularInDepth/angularindepth/image/16/3.png?raw=true)
...
![Console_4](https://github.com/AngularInDepth/angularindepth/image/16/4.png?raw=true)

调用 `rxSpy.undo` 并传递与方法调用关联的数字将看到该调用的撤销函数被执行。例如，调用 `rxSpy.undo(3)` 将看到被标记为 `interval` 的 `Observable` 的记录被撤销之后的结果：

![Console_5](https://github.com/AngularInDepth/angularindepth/image/16/5.png?raw=true)
...
![Console_6](https://github.com/AngularInDepth/angularindepth/image/16/6.png?raw=true)

与 `log` 方法一样，`let` 可以撤消对该方法的调用：

![Console_7](https://github.com/AngularInDepth/angularindepth/image/16/7.png?raw=true)

能够在调试时暂停一个 `Observable` 对我来说几乎是不可或缺的。调用 `rxSpy.pause` 将暂停一个标记的 `Observable`，并返回一个可用于控制和检查 `Observable` 的通知（`notifications`）的 `deck`：

![Console_8](https://github.com/AngularInDepth/angularindepth/image/16/8.png?raw=true)

在该 `deck` 上调用 `log` 将显示 `Observable` 是否暂停，并显示被暂停的通知（`notifications`）。（通知是 `Notification` 使用 `materialize` 操作符获得的rxjs实例）

![Console_9](https://github.com/AngularInDepth/angularindepth/image/16/9.png?raw=true)

在 `deck` 上调用 `step` 将发出一个被暂停住的通知（`notifications`）：

![Console_10](https://github.com/AngularInDepth/angularindepth/image/16/10.png?raw=true)

调用 `resume` 将发出所有被暂停的通知（`notifications`），并将恢复 `Observable`：

![Console_11](https://github.com/AngularInDepth/angularindepth/image/16/11.png?raw=true)

调用 `pause` 将看到 `Observable` 重新进入暂停状态：

![Console_12](https://github.com/AngularInDepth/angularindepth/image/16/12.png?raw=true)

很容易忘记将返回的 `deck` 分配给变量，因此控制台API包含一个 `deck` 方法，和 `undo` 方法行为相似。调用它将显示 `pause` 调用的列表：

![Console_13](https://github.com/AngularInDepth/angularindepth/image/16/13.png?raw=true)

调用它并传递与调用相关联的数字将返回相对应的 `deck` ：

![Console_14](https://github.com/AngularInDepth/angularindepth/image/16/14.png?raw=true)

像 `log` 和 `let` 的调用一样，`pause` 的调用也可以撤销。撤销 `pause` 的调用将看到标记的 `Observable` 恢复正常：

![Console_15](https://github.com/AngularInDepth/angularindepth/image/16/15.png?raw=true)
