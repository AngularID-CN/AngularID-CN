...
最后,让我们总结一下我们都学到了什么。Angular内部的所有组件都在称为`view`的数据结构中表示。Angular的解析器解析模板并创建绑定。每个绑定定义了一个需要更新的DOM元素的属性以及用于获取其值的表达式。在变更检测期间用于比较用途的先前值`previous values`存储在`view`的`oldValues`属性中的。在变更检测期间,Angular执行了绑定,计算表达式,将它们与先前的值进行比较,并在必要时更新DOM。在每个变更检测周期之后,Angular会运行同步检查以确保组件状态与用户界面同步。再次提醒,此检查是同步执行的,可能会抛出`ExpressionChangedAfterItWasChecked`错误
