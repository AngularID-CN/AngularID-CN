...
为了防止错误,我们需要确保在变更检测运行期间表达式返回的值和之后的检查是相同的。在我们的例子中,我们可以通过将赋值的部分移出`time`getter来实现:
...
然而若是通过此种方式实现,`time`的getter函数返回的的值将始终相同,但是我们仍旧希望更新该值。通过之前的了解我们得知,产生错误的检查会在变更检测周期后立刻同步执行。因此如果我们通过异步更新该值的方式,将能够实现错误的避免。因此为了每毫秒更新一次的目标我们可以使用`setInterval`函数并设置延迟为1毫秒,如下所示:
...
这样的实现方式解决了我们初识的问题,但是很不幸的是,这又带来了一个新的问题。所有计时事件(如setInterval,setTimeout)都会在Angular中触发变更检测。这意味着通过这种实现,我们最终将进入无限循环的变更检测周期之中。为避免这种情况,我们需要一种方法既执行`setInterval`又不是触发变更检测检测机制。幸运的是有一种方法可以做到这一点。要了解如何执行此操作,我们首先需要了解为什么`setInterval`触发了Angular中的变更检测。

### 利用zones实现的自动化变更检测

与React相反的是Angular中的变更检测机制可以由浏览器中的任何异步事件的发生而全自动触发。这一切全自动的理念都借由引入了`zone`概念的
`zone.js`库而变为现实。`zone`可能帮助实现了Angular中的变更检测机制,但`zone`本身不是Angular变更检测机制的一部分。事实上,[Angular就算没有`zone`也可以允许](https://blog.angularindepth.com/do-you-still-think-that-ngzone-zone-js-is-required-for-change-detection-in-angular-16f7a575afef)。`zone`作为一个库只是提供了一种拦截异步事件的方法(如setInterval,setTimeout)并通知Angular它们的发生。基于这些通知的基础上,angular运行变更检测。
...
现在的状况下我们在Angular`zone`外异步进行时间的更新。这样的方式保证了在变更检测期间和后续的同步检查中,time属性的`getter`函数返回相同的值。当Angular在下一个变更检测周期中读取time属性的值时,该值将会更形并且更新将反应到相应的UI上。

> 使用`NgZone`在Angular的`zone`之外运行一些代码以避免触发变更检测机制是一种常见的优化技术。

## 调试

您可能想知道是否有任何方法可以在Angular中查看`view`及其绑定,事实上是有的。`@angular/core`模块中有一个名为`checkAndUpdateView`的函数,它在组件树中的每个`view`(component)上运行,并对每个`view`执行检查。当我遇到任何有关变更检测有关的问题时,这个方法总是我开始调试的起点。

为了更好地了解这个方法,请自己去试试看调试的效果,前往这个[demo地址](https://angular-eobrrh.stackblitz.io/)并打开控制台开始你的旅程。在代码中找到该函数并放置一个断点,单击按钮以触发变更检测,检查`view`变量。以下是我在进行尝试时的记录:
...
我们之前聊到由于单向数据流的限制你无法在`组件进行变更检测期间组件已经被检查后`修改组建的值。大多数情况下,当Angular运行子组件的变更检测时数据更新的触发是通过共享服务或同步事件广播实现的。但是比较骚气的状况是,你也可以通过直接将父组件注入子组件中的方式于某个生命周期钩子中更新父组件的状态。下面的代码演示了此内容的实现:
...
要理解这种状况需要知道Angular在变更检测期间执行的操作及其顺序。不过好消息是我们已经知道可以通过`checkAndUpdateView`方法来达到我们的目的。以下是可以在其函数体中找到的一部分代码的:
...
正如你所见,Angular同样在变更检测期间触发了生命周期钩子。有趣的是,某些声明周期钩子在`Angular处理绑定时的渲染部分之前`被调用,另外一些钩子则会在之后被调用。下面的图表演示了当Angular运行父组件的变更检测时会发生什么:

<p align="center"> 
    <img src="../assets/135/9.png">
</p>

让我们一步步地去理解它。首先它更新了子组件的输入绑定`input bindings`,然后它再次调用了子组件上的`OnInit`,`DoCheck`和`OnChanges`生命周期钩子。这一过程是有道理的,因为它只是更新了输入绑定和Angular需要通知子组件其输入绑定的初始化已经完成了。_之后angular对当前组件进行了渲染_,在渲染完成后,将会执行对子组件的变更检测机制。这一过程意味着它基本上会在子视图上重复上述操作。最终它调用子组件上的`AfterViewChecked`和`AfterViewInit`生命周期钩子,让它知道它已被检查。
...
最后,让我们总结一下我们都学到了什么。Angular内部的所有组件都在称为`view`的数据结构中表示。Angular的解析器解析模板并创建绑定。每个绑定定义了一个需要更新的DOM元素的属性以及用于获取其值的表达式。在变更检测期间用于比较用途的先前值`previous values`存储在`view`的`oldValues`属性中的。在变更检测期间,Angular执行了绑定,计算表达式,将它们与先前的值进行比较,并在必要时更新DOM。在每个变更检测周期之后,Angular会运行同步检查以确保组件状态与用户界面相同步。再次提醒,此检查是同步执行的,可能会抛出`ExpressionChangedAfterItWasChecked`错误
