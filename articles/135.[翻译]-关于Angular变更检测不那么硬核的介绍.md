...
现在的状况下我们在Angular`zone`外异步进行时间的更新。这样的方式保证了在变更检测期间和后续的同步检查中,time属性的`getter`函数返回相同的值。当Angular在下一个变更检测周期中读取time属性的值时,该值将会更形并且更新将反应到相应的用户界面上。

> 使用`NgZone`在Angular的`zone`之外运行一些代码以避免触发变更检测机制是一种常见的优化技术。

## 调试

您可能想知道是否有任何方法可以在Angular中查看`view`及其绑定,事实上是有的。`@angular/core`模块中有一个名为`checkAndUpdateView`的函数,它在组件树中的每个`view`(component)上运行,并对每个`view`执行检查。当我遇到任何和变更检测有关的问题时,这个方法总是我开始调试的起点。
...
让我们一步步地去理解它。首先它更新了子组件的输入绑定`input bindings`,然后它再次调用了子组件上的`OnInit`,`DoCheck`和`OnChanges`生命周期钩子。这一过程是有道理的,因为它只是更新了输入绑定和Angular需要通知子组件其输入绑定的初始化已经完成了。_之后angular对当前组件进行了渲染_,在渲染完成后,将会执行对子组件的变更检测机制。这一过程意味着它基本上会在子视图上重复上述操作。最终它调用子组件上的`AfterViewChecked`和`AfterViewInit`生命周期钩子,让它知道它已被检查。
...
最后,让我们总结一下我们都学到了什么。Angular内部的所有组件都在称为`view`的数据结构中表示。Angular的解析器解析模板并创建绑定。每个绑定定义了一个需要更新的DOM元素的属性以及用于获取其值的表达式。在变更检测期间用于比较用途的先前值`previous values`存储在`view`的`oldValues`属性中的。在变更检测期间,Angular执行了绑定,计算表达式,将它们与先前的值进行比较,并在必要时更新DOM。在每个变更检测周期之后,Angular会运行同步检查以确保组件状态与用户界面的内容同步。再次提醒,此检查是同步执行的,可能会抛出`ExpressionChangedAfterItWasChecked`错误
