...
第一个 `view` 将是 host view 。（译者注：host view 对于其它 view 而言，）就像 Angular 为了托管我们的 app component 而创建的根组件（root component）。我们需要继续执行才能进入其子 view，而这个子 view 就是为 `AppComponent` 组件所创建的 view。探索一番。`component` 属性包含对 `App` 组件实例的引用，`nodes` 属性包含为 `App` 组件模板内的元素而创建的 DOM 节点的引用，而 `oldValues` 数组存储绑定表达式的结果。
