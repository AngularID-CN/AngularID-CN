...
Angular 社区中的每个开发者都对新一代的 Angular 渲染引擎 `Ivy` 的即将发布感到十分激动。
...
关于以上编译后内容可以写一整篇的博客来讲述它的数据结构以及 Angular 使用它的方式，还可以撰写一系列的博客来介绍`编译器如何使用 incremental DOM技术将模板呈现为指令集`。这些都是很有趣的主题，但是他们不是本篇博客的主题。
...
- 我们可以封装既存的代码模式让我们的组件更加整洁。有趣的是: 在 Angular2 尚未发布之前，我就写过一组特殊的装饰器，它们依赖于规约并让缩短组件的声明，我称之为`嬉皮士模式`。 现在，Ivy 引擎让这一切再次成为可能
...
- 代码可能更加难以`follow`。在上面的示例中，如果你不知道`FromStore`的作用就可能会在理解如何创建`todos observable`时遇到 困难

- 代码变得难以工具化。上面的代码可能没有那么糟糕: `WebStorm/VSCode`还是可以在模板中使用自动补全功能，但是在其他地方可能因为过火的行为导致失去 Angular 已有的工具化便利

### 谨慎！

Angular团队目前的重点是让Ivy完全向后兼容，而关于如何进行元编程的官方文档将在之后发布。
